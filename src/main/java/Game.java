...
public class Game extends JFrame {
    ...
    void fillGameField(JPanel jPanel, int[][] matrix, boolean border) {
        ...
        for (int i = 0; i < _height; i++) {
            for (int j = 0; j < _width; j++) {
                if (matrix[j][i] == 0) {
                    ...
                    jPanel.add(createRectangle(this.rectangleColor, Color.black));
                }
            }
        }
    ...
    Component createRectangle(Color border) {
        JPanel _panel = new JPanel();
        _panel.setBorder(BorderFactory.createLineBorder(border));
        return _panel;
    }

    Component createRectangle(Color solid, Color border) {
        JPanel _panel = new JPanel();
